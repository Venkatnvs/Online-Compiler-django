...
def main_comp_all(codedata,lang,input_text):
    ...
    try:
        ...
        if lang=="py":
            if input_text:
                result = subprocess.run([str(settings.PATH_PYTHON),"data_out/codes/py/"+file_name+'.'+lang], input=input_text.encode(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            else:
                result = subprocess.run([str(settings.PATH_PYTHON),"data_out/codes/py/"+file_name+'.'+lang], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if lang=="js":
            result = subprocess.run(["node","data_out/codes/js/"+file_name+'.'+lang], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if lang=="cpp":
        ...
        if lang=="php":
            result = subprocess.run(["C:/xampp/php/windowsXamppPhp/php.exe","data_out/codes/php/"+file_name+'.'+lang], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if result.returncode == 0:
